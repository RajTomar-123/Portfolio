    ...
    id="About" className='w-full px-[12%] py-10 scroll-mt-20'>
      ...
      className='flex w-full flex-col lg:flex-row items-center gap-20 my-20'>
        ...
        className='w-64 sm:w-80 rounded-3xl max-w-none'>
            <Image src={assets.user_img} alt='user' className='w-70 rounded-3xl' />
        </motion.div>
        ...
        className='flex-1'>
            ...
            className='grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-2xl'>
                {infoList.map(({icon, iconDark, title, description}, index)=>(
                    ...
                    className={`rounded-xl p-6 cursor-pointer hover:-translate-y-1 duration-500 hover:shadow-[4px_4px_0_#000] ${isDarkMode ? "border border-white hover:shadow-white hover:bg-[#2a004a]" : "border-[0.5px] border-gray-400 hover:bg-[#fcf4ff]"}`}
                      key={index}>
